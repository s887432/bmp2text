...
int save2file(unsigned char *bmp, int size, char* filename)
{
	FILE *fp;
	int imageIdx = 0;
	...
	{
		if( saveIdx != 0 && saveIdx%16==0 )
		{
			fprintf(fp, "\n");
		}
		fprintf(fp, "0x%02X", bmp[imageIdx]);
		if( saveIdx != 15 )
		fprintf(fp, ", ");
		saveIdx++;
	}
...
unsigned char *LoadBitmapFile(char *filename, BITMAPINFOHEADER *bitmapInfoHeader)
{
    ...
    save2file(bitmapImage, bitmapInfoHeader->biSizeImage, "output.c");
...
int main(int argc, char *argv[])
{
	...
	{
		printf("Usage:\n");
		printf("\tbmp_reader bitmap_file output_file\n");
		exit(5);
	}
